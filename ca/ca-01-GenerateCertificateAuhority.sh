...
cat > openssl.cnf <<-EOF
	...
	name_opt          = ca_default
	cert_opt          = ca_default
	default_days      = $CA_INTERMEDIATE_DAYS
	preserve          = no
	...
	commonName_default              = $CA_ORGANIZATION Root CA
...
printHead "Generate root keys"
openssl genrsa -aes256 -out private/ca.key.pem 4096
chmod 400 private/ca.key.pem

printHead "Create root certificate"
openssl req -config openssl.cnf -key private/ca.key.pem -new -x509 -days $CA_ROOT_DAYS -sha256 -extensions v3_ca -out certs/ca.cert.pem
